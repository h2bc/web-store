...
An API route or workflow SHALL be tested in `api/tests/` through the Medusa integration runner. Pure storefront logic such as schemas, formatting and data-layer helpers SHALL be tested in `front/tests/` with Vitest. A user journey SHALL be tested in the root `tests/` with Playwright, and only journeys. The test rule SHALL state this and load whenever a file in one of those directories is touched.
...
There SHALL be one test file per route, module or journey, named after it, ending in `.test.ts`. Each test name or step title SHALL state the behaviour from the shopper's or the store owner's point of view, in their words, and each spec scenario SHALL map to one test named after it. Every test SHALL follow Given-When-Then: in Jest and Vitest as three blocks separated by blank lines, in Playwright as three steps titled with it. A test file SHALL hold only tests; support code SHALL live in a `support/` folder next to the tests, page objects, fixtures and data for Playwright in `tests/support/`, and Playwright tests SHALL import `test` from its fixtures file.
