...
A git pre-commit hook SHALL run each app's ESLint and Prettier check on the files staged under that app, on every commit, from the agent or from a terminal, and refuse the commit when either fails. Files that are not staged SHALL NOT be checked, so an unrelated dirty file neither blocks nor is gated by the commit. `pnpm i` at the root SHALL point git at the hook, so no one sets it up by hand. A bypass flag on the git command SHALL NOT be used.

#### Scenario: Commit with a lint error
- **WHEN** `git commit` runs while a staged file in either app fails lint or is unformatted
- **THEN** the hook refuses and no commit is created

#### Scenario: Commit beside an unrelated dirty file
- **WHEN** `git commit` runs while an unstaged file in either app fails lint and every staged file passes
- **THEN** the hook accepts and the commit is created
