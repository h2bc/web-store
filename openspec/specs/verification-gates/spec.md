...
The root `package.json` SHALL expose one script per check, each chaining both apps: `lint`, `format` and `format:check`, alongside the existing `typecheck:*` and `test:*` scripts. Each app SHALL expose `lint`, `format`, `format:check`, `typecheck` and `test`. ESLint and Prettier SHALL be dependencies of the apps only, never of the root.

#### Scenario: Root script runs both apps
- **WHEN** `pnpm lint` or `pnpm format:check` runs at the root
- **THEN** the api script runs first, then the storefront's, and the command exits non-zero if either fails

#### Scenario: Format check
- **WHEN** `pnpm format:check` runs on a tree with an unformatted file inside an app
...
A git pre-commit hook SHALL run `pnpm lint && pnpm format:check` on every commit, from the agent or from a terminal, and refuse the commit when either fails. `pnpm i` at the root SHALL point git at the hook, so no one sets it up by hand. A bypass flag on the git command SHALL NOT be used.
...
CI SHALL check a push or pull request with four steps, in order: Format (`pnpm format:check`), Lint (`pnpm lint`), Typecheck (the root, api and storefront typecheck scripts) and Tests (`pnpm test:api && pnpm test:e2e`). The API integration tests SHALL run against a database service. The image build jobs SHALL depend on this job.
...
A task's verify clause SHALL name a root script (`pnpm lint`, `pnpm format:check`, `pnpm typecheck:api`, `pnpm typecheck:front`, `pnpm test:api`, `pnpm test:e2e`) or an observable behaviour, never a tool invoked directly. The last task of a change SHALL run lint, the format check, both app typechecks and the API tests.

#### Scenario: Task list written for an API change
- **WHEN** a tasks.md is created for a change that touches `api/`
- **THEN** its last task runs `pnpm lint`, `pnpm format:check`, `pnpm typecheck:api`, `pnpm typecheck:front` and `pnpm test:api`
