...
|---|---|
| Owner is told on Telegram when an order is placed | 1.1, 1.2, 1.3, 2.1, 2.2, 2.3 |
| Telegram delivery is optional and switched on by configuration | 1.2, 1.4, 2.3 |
| Owner receives an order-placed email | 2.1, 2.2, 2.4, 2.5 |
...
- [ ] 1.1 Create `api/tests/order-notifications.test.ts` (integration, Medusa runner with `env` carrying `ORDER_INBOX_EMAIL`, `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID`) with a `support/notifications.ts` helper that creates an order through the order module, stubs the global `fetch` and lists notifications from the notification module; add the Telegram scenarios from `owner-notifications`: message recorded for the chat id with the order data, second delivery records nothing, order without email still records it, Bot API error is logged and the order still succeeds; verify the tests fail because no `telegram` provider exists.
...
- [ ] 2.1 Add the owner email scenarios from `transactional-emails` to `api/tests/order-notifications.test.ts`: owner email recorded for the inbox with `admin_url`, second delivery records nothing, guest order without email records the owner email but not the customer one, inbox unset records no owner email; verify they fail because the workflow sends only the customer email.
- [ ] 2.2 In `api/src/workflows/send-order-confirmation.ts` add a `getNotificationRecipients` step reading `ORDER_INBOX_EMAIL` and `TELEGRAM_CHAT_ID`, and build the notification list from the order and the recipients: customer `order-placed` when the order has an email, `order-placed-owner` email with `admin_url` when the inbox is set, `order-placed` on the `telegram` channel when the chat id is set, each with its own idempotency key; verify the 1.1 and 2.1 tests pass.
...
- [ ] 2.4 Create `api/src/modules/resend/emails/order-placed-owner.tsx` on `EmailLayout` rendering the admin link, items with variant and quantity, the shipping address, the shipping method and the totals, and register `ORDER_PLACED_OWNER` in `api/src/modules/resend/service.ts` with the subject `New order #<display id>, <total>`; verify `pnpm typecheck:api` passes.
- [ ] 2.5 Add an `order-placed-owner` case to `api/src/scripts/test-email.ts` reusing the mock order plus an `admin_url`; verify `npx medusa exec ./src/scripts/test-email.ts <address> order-placed-owner` sends it.
