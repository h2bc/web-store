...
- [x] 2.5 In `tests/cart.test.ts` add a journey that adds a product and expects the cart cookie to be `HttpOnly` and `SameSite=Lax`, and see it fail; set `sameSite: 'lax'` and `secure` in production on the cart cookie in `front/lib/cookies.ts` and `poweredByHeader: false` in `front/next.config.ts`; verify the journey passes and a page response carries no `X-Powered-By`.
