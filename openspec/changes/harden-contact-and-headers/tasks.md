...
- [ ] 2.3 In `front/lib/seo.ts` make `siteUrl` and `isIndexable` async with `await connection()` from `next/server` before the environment read, and await them in `front/app/layout.tsx`, `front/app/robots.ts`, `front/app/sitemap.ts`, the product page and `productJsonLd`'s caller; verify `pnpm typecheck:front` passes.
- [ ] 2.4 Delete `export const dynamic = 'force-dynamic'` from `front/app/layout.tsx`, `front/app/robots.ts`, `front/app/sitemap.ts`, `front/app/(main)/checkout/page.tsx` and `front/app/(main)/checkout/return/page.tsx`; verify `grep -rn "force-dynamic" front/app` finds nothing, `pnpm --dir front build` succeeds with no page marked static, and `pnpm test:front` and `pnpm lint:front` pass.
- [ ] 2.5 With both apps up, open `http://localhost:3000/`, a product page, the gallery, the contact page and the checkout payment step in the Playwright MCP browser; open a select and trigger a toast; verify the response carries the eight headers, the page source shows the nonce on the framework scripts, the browser console shows no CSP violation, `robots.txt` and `sitemap.xml` use the site URL from the environment, the YouTube player and the Stripe elements render, and a screenshot of the payment step is saved under `.tmp/`.

## 3. Docs

- [ ] 3.1 In `docs/architecture.md`, under Storefront, add that `front/proxy.ts` sets the security headers and the content policy with a nonce per request, and that runtime config reads call `connection()` instead of a route flag; under Backend, add that the contact limiter keys on the first forwarded address because the storefront calls the API through the proxy; verify `grep -n "Content-Security-Policy\|connection()\|first forwarded" docs/architecture.md` finds three lines.
