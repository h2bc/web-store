...
- [x] 2.5 In `front/tests/cart-cookie.test.ts` expect `getCartCookieOptions` from `front/lib/cart-cookie.ts` to return `HttpOnly`, `SameSite=Lax` and, in production, `Secure`, and see it fail; in `front/tests/next-config.test.ts` expect `poweredByHeader` to be `false`; add the helper, use it in `front/lib/cookies.ts`, and set `poweredByHeader: false` in `front/next.config.ts`; verify `pnpm test:front` passes and a page response carries no `X-Powered-By`.
