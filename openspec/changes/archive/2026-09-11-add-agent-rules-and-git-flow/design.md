...
- Which verify runs where? The typechecks and API tests at commit time; `/opsx:verify` at PR time, refusing on CRITICAL except for outside-repo tasks and the tasks that verify against the PR itself, which are unticked by definition when it opens.
