...
- [x] 6.1 In `h2bc/web-store-deploy`, set `MEDUSA_BACKEND_URL` to the public API origin (for dev `https://api.dev.h2bcweb.com` or whatever the API is served from, no trailing slash) on every API container, server and worker alike; verify by inviting a test user from the deployed admin and checking the received email's link opens the deployed dashboard
- [x] 6.2 In `h2bc/web-store-deploy`, confirm `RESEND_API_KEY`, `RESEND_FROM_EMAIL` and `EMAIL_LOGO_URL` are set on the worker container, not only the server; verify by placing a test order on the deployed storefront and receiving the confirmation email
- [x] 6.3 In the Resend dashboard, confirm the sending domain for `RESEND_FROM_EMAIL` is verified; verify the test emails from 6.1 and 6.2 arrive in the inbox rather than being rejected
