...
**Payment methods come from the Stripe Dashboard.** The Medusa Stripe provider is configured with `automaticPaymentMethods: true`, so every PaymentIntent uses Stripe's automatic payment methods and the Payment Element offers whatever is enabled in the Stripe Dashboard, including card, Apple Pay and Google Pay. The storefront passes no payment method data when initiating the session. `confirmPayment` runs with `redirect: 'if_required'` and a `return_url` of `/checkout/return`. Card, wallets and in-app approval methods resolve on the same page and `completeCart` runs next. Redirect methods come back to the return page, which retrieves the PaymentIntent with Stripe.js and completes the cart when it succeeded, shows the last payment error with a link back to the payment step when it failed, or releases the cart and explains that confirmation arrives by email when the bank is still processing it. Stripe's webhook also completes the cart once the payment is captured, so `completeCart` no longer refuses a completed cart; Medusa's complete-cart workflow returns the existing order for a completed cart without charging again. Billing details are supplied to `confirmPayment` from the cart's address so the Payment Element does not ask for name, email or address again. Alternative: pin the PaymentIntent to `payment_method_types: ['card']` from the storefront. Rejected; the storefront would need a code change for every payment method decision made in the Dashboard.
