...
- The contact action returns the success message as data and a message as error, without per-field errors. The form already validates every field with the same schema before submitting. The shipped contact form email change already dropped the per-field errors and made the action return `{ error }`; task 1.5 moves it to `Result<string>`.
